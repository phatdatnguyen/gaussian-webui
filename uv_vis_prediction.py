...
def on_create_molecule(input_smiles_textbox: gr.Textbox):
    ...
    try:
        global mol_uv_vis
        mol_uv_vis = Chem.MolFromSmiles(input_smiles_textbox)
        mol_uv_vis = Chem.AddHs(mol_uv_vis)
...
def on_upload_molecule(load_molecule_uploadbutton: gr.UploadButton):
    ...
    try:
        ...
        if file_extension.lower() == ".pdb":
            mol_uv_vis = Chem.MolFromPDBFile(uploaded_file_path, sanitize=False, removeHs=False)
        elif file_extension.lower() == ".xyz":    
            mol_uv_vis = mol_from_xyz_file(uploaded_file_path)
        elif file_extension.lower() == ".mol":    
            mol_uv_vis = Chem.MolFromMolFile(uploaded_file_path, sanitize=False, removeHs=False)
        elif file_extension.lower() == ".mol2":    
            mol_uv_vis = Chem.MolFromMol2File(uploaded_file_path, sanitize=False, removeHs=False)
...
def write_uv_vis_gaussian_input(mol, file_name, method='B3LYP', basis='6-31G(d,p)', n_states=10, solvation=None, solvent=None, charge=0, multiplicity=1, n_proc=4, memory=4):
    ...
    with open(file_name + '.gjf', 'w') as f:
        ...
        f.write(f'%Mem={memory}GB\n')
        f.write(f'%Chk={file_name}.chk\n')
        route2 = f'#P {method}/{basis} TD(NStates={n_states}) Geom=AllCheck Guess=Read'
        if solvation:
    ...
    print(f"Wrote chained Gaussian input '{file_name}.gjf' (Opt -> TD).")
...
def generate_uv_vis_spectrum_interactive(wavelengths, oscs, points=10000, plot_range=None):
    ...
    try:
        if mm_checkbox:
            if force_field_dropdown=="MMFF":
                AllChem.MMFFOptimizeMolecule(mol_uv_vis, maxIters=max_iters_slider)
            else:
                AllChem.UFFOptimizeMolecule(mol_uv_vis, maxIters=max_iters_slider)
...
def uv_vis_prediction_tab_content():
    with gr.Tab("UV-Vis Spectrum Prediction") as uv_vis_prediction_tab:
        ...
        with gr.Accordion("UV-Vis Spectrum Prediction"):
            with gr.Row(equal_height=True):
                with gr.Column(scale=1):
                    with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    file_name_textbox = gr.Textbox(label="File name", value="molecule_uv_vis")
