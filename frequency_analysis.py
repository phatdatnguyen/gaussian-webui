...
def on_upload_molecule(load_molecule_uploadbutton: gr.UploadButton):
    ...
    try:
        ...
        gr.Warning("Error!\n" + str(exc))
        return None, None, gr.update(interactive=False), None, None, None, None, None, gr.update(interactive=False), None, None, None, None
    ...
    if file_extension.lower() == ".log":
        try:
            ...
            return smiles, html, gr.update(interactive=True), energy_textbox, dipole_moment_textbox, energy_plot, html, visualization_dropdown, gr.update(interactive=True), "", MO_df, ir_spectrum, thermo_html
        except:
            return smiles, html, gr.update(interactive=True), None, None, None, None, None, gr.update(interactive=False), None, None, None, None
    else:
        return smiles, html, gr.update(interactive=True), None, None, None, None, None, gr.update(interactive=False), None, None, None, None
...
def write_opt_freq_gaussian_input(mol, file_name, method='B3LYP', basis='6-31G(d,p)', charge=0, multiplicity=1, solvation=None, solvent=None, n_proc=4, memory=2):
...
def on_frequency_analyze(mm_checkbox: gr.Checkbox, force_field_dropdown: gr.Dropdown, max_iters_slider: gr.Slider, solvation_checkbox: gr.Checkbox, solvation_dropdown: gr.Dropdown, solvent_dropdown: gr.Dropdown,
    ...
    try:
        ...
        if solvation_checkbox:
            ...
            solvation = None
        write_opt_freq_gaussian_input(mol_freq, file_name=file_name_textbox, method=functional_textbox, basis=basis_set_textbox, charge=charge_slider, multiplicity=multiplicity_dropdown, solvation=solvation, solvent=solvent_dropdown, n_proc=n_cores_slider, memory=memory_slider)
        ...
        return None, gr.update(visible=False), None, None, None, None, None, gr.update(interactive=False), None, None, None, None

    calculation_status = "Optimization finished. ({0:.3f} s)".format(duration)
    output_file_path = f'{file_name_textbox}.log'
    return calculation_status, gr.update(value=output_file_path, visible=True), energy_textbox, dipole_moment_textbox, energy_plot, html, visualization_dropdown, gr.update(interactive=True), "", MO_df, ir_spectrum, thermo_html
...
def frequency_analysis_tab_content():
    with gr.Tab("Frequency Analysis") as frequency_analysis_tab:
        ...
        load_molecule_uploadbutton.upload(on_upload_molecule, load_molecule_uploadbutton, [smiles_texbox, molecule_viewer, analyze_button, energy_texbox, dipole_moment_texbox, energy_plot, conformers_viewer, visualization_dropdown, visualize_button, visualization_html, MO_dataframe, ir_spectrum_plot, thermo_html])
        ...
        analyze_button.click(on_frequency_analyze, [mm_checkbox, force_field_dropdown, max_iters_slider, solvation_checkbox, solvation_dropdown, solvent_dropdown,
                                                    functional_textbox, basis_set_textbox, charge_slider, multiplicity_dropdown,
                                                    n_cores_slider, memory_slider, file_name_textbox],
                                                   [status_markdown, download_button, energy_texbox, dipole_moment_texbox, energy_plot, conformers_viewer, visualization_dropdown, visualize_button, visualization_html, MO_dataframe, ir_spectrum_plot, thermo_html])
